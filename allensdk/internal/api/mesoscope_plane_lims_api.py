...
class MesoscopePlaneLimsApi(BehaviorOphysLimsApi):
    ...
    def get_max_projection(self, image_api=None):
        ...
        if self.get_surface_2p_pixel_size_um() == 0:
            pixel_size = 400/512 #hardcoded as not being written from the rig, to be fixed by MPE
    ...
    def get_segmentation_mask_image(self, image_api=None):
        ...
        if self.get_surface_2p_pixel_size_um() == 0:
            pixel_size = 400/512 #hardcoded as not being written form the rig, to be fixed by MPE
        else : pixel_size = self.get_surface_2p_pixel_size_um()
    ...
    def get_licks(self): #read licks from sync, if they are absent, read from pickle
